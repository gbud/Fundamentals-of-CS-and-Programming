...
def nearestOdd(n):
    if n > 0 and n % 2 < 1 and n % 2 > 0:
        return int(n) + 1
    elif n < 0 and n % 2 < 1 and n % 2 > 0:
        return int(n)
    elif n % 2 > 1 and n > 0:
        return int(n)
    elif n % 2 > 1 and n < 0:
...
def numberOfPoolBallRows(balls):
    low = (-1 - math.sqrt(1 + 8*balls)) / 2
    high = (-1 + math.sqrt(1 + 8*balls)) / 2
    if (balls - high) % 1 == 0:
        return int(high)
    else:
        return int(abs(low))
...
def colorBlender(rgb1, rgb2, midpoints, n):
    ...
    n=int(n)
    #new color = original color - n(difference)/(mid+1)
    dif = n/(midpoints+1)

    red1 = rgb1//10**6
    red2 = rgb2//10**6
    newred = roundHalfUp(red1 - (red1-red2)*dif)
    if newred <= 99 and newred > 9:
        printred = ("0" + str(newred))
    elif newred <=9:
        printred = ("00" + str(newred))
    else:
        printred = (str(newred))

    green1 = rgb1//10**3 % 10**3
    green2 = rgb2//10**3 % 10**3
    newgreen = roundHalfUp(green1 - (green1-green2)*dif)
    if newgreen <= 99 and newgreen > 9:
        printgreen = ("0" + str(newgreen))
    elif newgreen <=9:
        printgreen = ("00" + str(newgreen))
    else:
        printgreen = (str(newgreen))

    blue1 = rgb1 % 10**3
    blue2 = rgb2 % 10**3
    newblue = roundHalfUp(blue1 - (blue1-blue2)*dif)
    if newblue <= 99 and newblue > 9:
        printblue = ("0" + str(newblue))
    elif newblue <=9:
        printblue = ("00" + str(newblue))
    else:
        printblue = (str(newblue))
    
    if n < 0 or n > midpoints +1:
        return None
    else:
        return int(printred+printgreen+printblue)
