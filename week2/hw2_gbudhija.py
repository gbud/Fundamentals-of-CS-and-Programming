...
def isPrime(n): #fasterPrime from class notes
    if (n < 2):
        return False
    if (n == 2):
        return True
...
def isSmith(n):
    if isPrime(n) == True:
        return False
    sumDig = sumOfDigits(n)
    factN = n
    primeSum = 0
    nPrime = 0
    currPrime = 2
    for factN in range(n):
        if n % currPrime == 0:
            factN = n // currPrime
            primeSum += sumOfDigits(currPrime)
            if (isPrime(factN) == True) and (factN > 9):
                primeSum += sumOfDigits(factN)
                if primeSum == sumDig:
                    return True
                else: return False
            elif (isPrime(factN) == True) and ((primeSum + factN) == sumDig):
                    return True
            else:
                n = factN
        else:
            nextDigit = currPrime +1
            if isPrime(nextDigit) == True:
                currPrime = nextDigit
            else:
                currPrime += 1
    return False
...
def mostFrequentDigit(n):
    ...
    for digit in range(10):
        if countTheDigits(n, digit) > bestCount:
            bestCount = countTheDigits(n, digit)
            best = digit
    return best

def isRotation(x, y):
    numbLeng = digitCount(x)
    for i in range (numbLeng):
        if x == y:
            return True
        newFront = x % 10
        x //= 10
        x += newFront * (10**(numbLeng - 1))
    return False
...
def findZeroWithBisection(f, x0, x1, epsilon):
    ...
    if (f(x0) < 0 and f(x1) < 0) or (f(x1) > 0 and f(x0) > 0): return None
    xMid = (x1 + x0) / 2
    while abs(f(xMid)) >= epsilon:
        if (f(xMid) > 0 and f(x0) > 0) or (f(xMid) < 0 and f(x0) < 0): #xo same
            x0 = xMid
            newxMid = (x1 + x0) / 2
            xMid = newxMid
            if abs(f(xMid)) <= epsilon:
                ...
                continue
        elif (f(xMid) > 0 and f(x1) > 0) or (f(xMid) < 0 and f(x1) < 0): #x1 sm
            x1 = xMid
            newxMid = (x1 + x0) / 2
            xMid = newxMid
            if abs(f(xMid)) <= epsilon:
...
def carrylessAdd(x1, x2):
    dcx1 = digitCount(x1)
    dcx2 = digitCount(x2)
    maxDigits = max(dcx1,dcx2)
    total = 0
    for d in range(maxDigits):
        digit1 = getKthDigit(x1,d)
        digit2 = getKthDigit(x2,d)
        sumDig=(digit1 + digit2) % 10
        total += sumDig * 10**d
    return total
...
def playPig():
    ...
    player1score = 0
    player2score = 0
    input("Welcome " + player1 + " & " + player2 + "!!!")
    while player1score <=100 or player2score <= 100:
        player1score += playerTurn(player1,player1score)
        print(player1,"'s total score: ",player1score)
        player2score += playerTurn(player2,player2score)
        print(player2,"'s total score: ",player2score)

def playerScore(player):
    score = 0
    return score

def playerTurn(player,playerScore):
    roundTotal = 0
    while roundTotal <= 100:
        turn = input("its " + str(player) + "'s turn (r = roll, p = pass):")
        if str(turn) == "r":
            roll = (diceRoll())
            print(roll)
            if roll > 1:
                print("round total:", roundTotal + roll)
                roundTotal += roll
                if playerScore + roundTotal >= 100:
                    print(player,"'s total score: ",(playerScore+roundTotal))
                    print(player,"WINS!!!")
                else:
                    continue
            else:
            ...
            print("passss,", player, "'s score this round: ", roundTotal)
            return roundTotal  
